...
if strcmp(para.mask_method, 'quality')
    disp('quality masking')
    qmask = nifti('quality.nii') ;
    qmask = qmask.dat(:,:,:) ;
    qmask(~isfinite(qmask)) = 0;
    qmask(qmask>para.qmask_thr) = 1 ;
    qmask(qmask<=para.qmask_thr) = 0 ;
    qmask = int16(qmask) ;
    createNifti(qmask, 'mask.nii', Maff_image)
    clear qmask
elseif strcmp(para.mask_method, 'spm_quality')

    % spm mask
    fill_holes = 'yes';
    conn = 8;
    mask_mag = true;
    [~, mask] = mask_spm12(mag_files(1,:), para.spm_mask_thr, fill_holes, conn, mask_mag);
    ...
    % making joined spm and quality mask
    createNifti(qmask.*int16(mask), 'mask.nii', Maff_image)
    clear qmask mask

elseif strcmp(para.mask_method, 'spm')

    fill_holes = 'yes';
    conn = 8;
    mask_mag = true;
    [mask_file, ~] = mask_spm12(mag_files(1,:), para.spm_mask_thr, fill_holes, conn, mask_mag);
    copyfile(mask_file, 'mask.nii')

elseif strcmp(para.mask_method, 'load') && ~strcmp(para.mask_file, fullfile(pwd,'mask.nii'))
    copyfile(para.mask_file, 'mask.nii')
end
